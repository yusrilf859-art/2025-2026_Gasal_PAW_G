...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        $stmt = $koneksi->prepare("UPDATE supplier SET nama = ?, telp = ?, alamat = ? WHERE id = ?");
        $stmt->bind_param("sssi", $nama, $telp, $alamat, $id);

        if ($stmt->execute()) {
            $_SESSION['pesan'] = "Data supplier berhasil di-update.";
    ...
    $stmt = $koneksi->prepare("SELECT nama, telp, alamat FROM supplier WHERE id = ?");
    ...
    if ($result->num_rows == 1) {
        ...
        $_SESSION['pesan'] = "Data supplier tidak ditemukan.";
        header("Location: index.php");
        exit();
    }
    $stmt->close();
...
<body>
    <div class="container">
        ...
        <form action="edit.php?id=<?php echo $id; ?>" method="POST" class="form-supplier" novalidate>
